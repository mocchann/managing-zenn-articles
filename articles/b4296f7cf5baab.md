...
書籍には各章ごとに練習問題があるのですが、それを1つの解法だけでなく、**複数パターンの解答を考え実装する**ことで色々な方法を知ることができるためオススメです

また、TypeScriptの学習をはじめた少しあとに社内でもSPAアプリケーションを触れる人材を増やすことを目的に、TypeScript + Reactの社内勉強会が行われていました
...
:::message
**何かしらの課題の解決方法を自分の頭で考えて手を動かす**
**→課題を目の前にすると思うように手が動かない**
**→わからないことを調べて、再度手を動かして課題を解決する**
:::
...
また、内容もただ読むだけではなく手を動かしつつ、自分の頭を使って考える問題が用意されていてインプットとアウトプットを短いサイクルで繰り返しながら動作検証ができるのが良かったです
...
技術のキャッチアップのたびに思うのですが、**学んだ技術を使って一から自分で何かしらのアプリを作る**のが一番です
...
ただ、エンジニアとして仕事をしていると自分知らない技術がいくつも重なって**何がどうなっているのかわからない状態**になるという経験があると思います(特にエンジニアとして働きはじめたときはよくなっていました。。)

そんなときは一旦立ち止まって、以下のように**現状の把握理**と**次にどんなアクションをすれば良さそうか**を整理してから行動することをオススメします

:::message
1. 現状どういう状態になっているか
2. どこまで理解できているか
3. 何がわからないか
4. どういうアクションをすれば課題解決できそうか
5. 上記を含めて次のアクションをどうするか
:::
