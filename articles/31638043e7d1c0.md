...
集約（Aggregate）のルート: 多くのエンティティは、関連するオブジェクト（他のエンティティや値オブジェクト）をまとめた「集約」のルート（Aggregate Root）としての役割を果たし、整合性を保つ単位となる
...
エンティティはドメインの関心ごとに集中するべきであるため、DBテーブルの構造などに引きずられるべきでない
リポジトリパターンは、エンティティと永続化の技術詳細を分離するための重要な仕組み
